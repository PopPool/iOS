...
final class AdminBottomSheetView: UIView {
    ...
    let segmentedControl: PPSegmentedControl = {
        let control = PPSegmentedControl(type: .tab, segments: ["상태값", "카테고리"], selectedSegmentIndex: 0)
        return control
    }()
    ...
    func updateContentVisibility(isCategorySelected: Bool) {
        ...
        UIView.animate(withDuration: 0.3) {
            self.contentHeightConstraint?.update(offset: newHeight)
            self.contentCollectionView.reloadData()
            self.layoutIfNeeded()
        }
    }

    func update(statusText: String?, categoryText: String?) {
        var filters: [String] = []

        if let statusText = statusText, !statusText.isEmpty {
            filters.append(statusText)
        }
        ...
        resetButton.isEnabled = hasFilters
        saveButton.isEnabled = hasFilters
    }

    func updateCategoryButtonSelection(_ category: String) {
        contentCollectionView.visibleCells.forEach { cell in
            guard let tagCell = cell as? TagSectionCell else { return }
            // input으로 상태 업데이트
            let input = TagSectionCell.Input(
                title: category,
                isSelected: !tagCell.contentView.backgroundColor!.isEqual(UIColor.blu500),
                id: nil
            )
            tagCell.injection(with: input)
        }
    }
        }
